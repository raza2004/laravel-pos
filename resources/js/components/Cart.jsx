...
const yoloUrl = 'https://www.yolofood.pk';
class Cart extends Component {
    ...
    loadProducts(search = "") {
        ...
        axios.get(`${yoloUrl}/api/restorant/19/items`).then((res) => {
            // console.log('products',res.data.data[5])
            const products1 = res.data.data[0];
            const products2 = res.data.data[1];
            const products3 = res.data.data[2];
            const products4 = res.data.data[3];
            const products5 = res.data.data[4];
            const products6 = res.data.data[5];
            const products=products1.concat(products2,products3,products4,products5,products6);
    ...
    searchProduct = async (recBarcode) => {
        let filteredArray = []
        let actualData = this.state.products
    ...
    deleteItemFromCart = async (recBarcode) => {
        ...
        for (let i = 0; i < actualData.length; i++) {
            if (actualData[i].id === Number(recBarcode)) {
    ...
    addProductToCart(data) {
        let product = this.state.products.find((p) => p.id === data.id);
        if (!!product) {
            console.log('name > ', product.name)
            // if product is already in cart
            let cart = this.state.cart.find((c) => c.id === data.id);
            console.log('Cart  ', this.state.cart)
            if (!!cart) {
                // update quantity
                // console.log('update qty ')
                this.setState({
                    cart: this.state.cart.map((c) => {
                        if (c.id === data.id && data.available > c.pivot.quantity) {
                            c.pivot.quantity = c.pivot.quantity + 1;
                        } else if (c.id === data.id && data.available === c.pivot.quantity) {
                            Swal.fire("Error!", "Stock Limit Exceeded", "error");
                        }
                        return c;
                    }),
    ...
    printBill() {
        ...
        Swal.fire({
            ...
            html: `
            <button type="button" onClick=${this.printPreview} style="margin-right:-80%; background-color:blue; border-radius:3%; color:white; width: 17%;padding:2px;">Print</button>
            <div id="slip1">
            ...
            <table style="border-spacing: 30px;font-family: arial, sans-serif;width: 100%;margin-top:1px">
                ...
                <td >${item.pivot.quantity}.0</td>
                    ...
                    <td class="a" style="width:max-content">&nbsp${item.price}</td>
                    <td class="b" style:"margin-left:2%">&nbsp${item.pivot.quantity * item.price}</td>
                  </tr>`
            )).join('')}
                ...
                <tr style=font-size: 13px; text-align: left;">
                  ...
                  <td></td>
                  <td></td>
                  <td><b>Rs. ${this.getTotal(this.state.cart)}</b></td>
                </tr>
              </table>
              <br/><br/>
    ...
    //         if (products[i].id === 778) {
    //             filteredArrays.push(products[i])
    //         }
    //     };
                                ...
                                <div className="card">
                                    <table className="table table-striped">
                                        <thead className="thead">
                                            <tr>
                                                <th className="thmy">Product Name</th>
                                        ...
                                        <tbody>
                                            {cart.map((c) => (
                                                <tr key={c.id} className="rite">
                                                    <td className="tdmy">{c.name.toUpperCase()}</td>
                                                        ...
                                                        <input type="text"
                                                            ...
                                                            onChange={
                                                                (event) => {
                                                                    ...
                                                                    this.handleChangeQty(c.id, event.target.value);
                                                                }
                                                            }
                            ...
                            <div className="order-product">
                                {
                                    this.state.searchedData.length > 0 ? (
                                        this.state.searchedData.slice(0, 56).map((d) => (
                                            <div
                                                style={{}}
                                            ...
                                            >
                                                ...
                                                <p style={{ fontSize: 12, textAlign: 'center' }}>
                                                    <b> {d.name.toUpperCase().substring(0, 25)}</b>
                                        ...
                                        this.state.products.slice(0, 226).map((d) => (
                                            <div
                                                style={{}}
                                                onClick={() => { this.addProductToCart(d) }}
                                                key={d.id}
                                            ...
                                            >
                                                ...
                                                <p style={{ fontSize: 12, textAlign: 'center' }}>
                                                    ...
                                                    <p style={{ fontSize: 11, textAlign: 'center' }}>Total Qty : {d.available}</p>
                                                    <p style={{ fontSize: 11, marginTop: -15, textAlign: 'center' }}>Price : Rs.{d.price}</p>
                                                </p>
                                            </div>
                                        )))
                                }
